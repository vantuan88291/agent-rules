...
1. **Create the symlink if it does not exist.** From this agent's workspace root, run:
   ...
   ```
   (Use the workspace path for this agent: e.g. on Pi `/home/vantuan88291/.openclaw/workspace/dev2`, so full command can be: `ln -sf ~/Documents/code/reactnative /home/vantuan88291/.openclaw/workspace/dev2/reactnative`. Or `cd` to workspace root then `ln -sf ~/Documents/code/reactnative reactnative`.)
